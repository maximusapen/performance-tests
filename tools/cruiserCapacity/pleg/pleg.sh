...
while (true); do
    if [[ $D_COUNT -le 0 ]]; then
        C_COUNT=$(sudo docker ps | wc -l)
        echo "`date +%Y%m%d-%H%M%S` containers $C_COUNT"
        time docker inspect `docker ps -qa` | wc -l
        D_COUNT=10
    else
        D_COUNT=$((D_COUNT-1))
    fi
    SECONDS=0
    LAST=0
    for i in `sudo docker ps | grep -v COMMAND | tee pleg.docker.lst | cut -d" " -f1`; do
        DUR1ST=$((SECONDS-LAST))
        sudo docker inspect $i > /dev/null
        if [[ $? -ne 0 ]]; then

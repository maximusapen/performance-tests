...
class Runner(object):
  ...
  def _kubectl(self, stdin, *args):
    ...
    if type(stdin) is str:
      # Only log stdin of str type.  One of the call is "Copying query files to client"
      # with a tar file which causes UnicodeDecodeError for python3:
  ...
  def _run_top(self, output_q):
    ...
    while time.time() < t_end:
      ...
      pcpu = re.findall(' \d+m ', perfout.decode())
      pmem = re.findall(' \d+Mi ', perfout.decode())
      kcpu = re.findall(' \d+m ', kubeout.decode())
      kmem = re.findall(' \d+Mi ', kubeout.decode())

...
function create-clusters() {
	...
	while ibmcloud ks clusters | grep -Fq 'deleting'; do
		echo -n "."
		sleep 10
	done
	echo ""
	ibmcloud ks init --host stage2.containers.test.cloud.ibm.com
	#ibmcloud ks region-set us-south >/dev/null
	#ibmcloud ks vlans dal09 >/dev/null
	...
	while ! ibmcloud ks clusters | grep 'kubeSpawnTester' | grep -Fq 'normal'; do
		echo -n "."
		sleep 5
	done
	while ! ibmcloud ks clusters | grep 'kubeMasterTester' | grep -Fq 'normal'; do
		echo -n "."
		sleep 5
	done
...
function push-image() {
	if [[ "${ISBUILD}" == "y" ]]; then
		...
		KUBEMARK_IMAGE_REGISTRY=$(echo "stg.icr.io/armada_performance")
		KUBE_NAMESPACE=""
	fi
}
...
function choose-clusters() {
	...
	if [[ "${USE_EXISTING}" == "y" ]]; then
		...
		if [ -z "${CUSTOM_MASTER_CONFIG}" ]; then
...
function set-registry-secrets() {
	spawn-config
	kubectl get secret default-stg-icr-io -o yaml | sed "s/default/kubemark/g" | kubectl -n kubemark create -f -
	kubectl patch -n kubemark serviceaccount/default -p "{\"imagePullSecrets\":[{\"name\": \"kubemark-stg-icr-io\"}]}"
}
...
function delete-clusters() {
	...
	while ! ibmcloud ks clusters | grep 'kubeSpawnTester' | grep -Fq 'deleting'; do
		sleep 5
	done
	while ! ibmcloud ks clusters | grep 'kubeMasterTester' | grep -Fq 'deleting'; do
		sleep 5
	done

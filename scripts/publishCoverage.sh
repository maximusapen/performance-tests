...
if [ "$TRAVIS_GO_VERSION" == "tip" ] || [ "$MAKE_TASK" != "swaggertravis" ]; then
...
OLD_COVERAGE=$(cat $TRAVIS_BUILD_DIR/gh-pages/coverage/$TRAVIS_BRANCH/cover_percent.out | awk '{s+=$1}END{print s/NR}')
...
cp $TRAVIS_BUILD_DIR/cover_percent.out $TRAVIS_BUILD_DIR/gh-pages/coverage/$TRAVIS_BRANCH
cp $TRAVIS_BUILD_DIR/cover_percent.out $TRAVIS_BUILD_DIR/gh-pages/coverage/$TRAVIS_COMMIT
NEW_COVERAGE=$(cat $TRAVIS_BUILD_DIR/cover_percent.out | awk '{s+=$1}END{print s/NR}')
